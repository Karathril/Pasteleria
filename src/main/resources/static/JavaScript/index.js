...
const clickAgregar = async (event) => {
    ...
    try {
        const respuesta1 = await fetch('http://localhost:8081/products/'.concat(id_producto));
...
function mostrarProductosEnCarrito() {
    ...
    datosArray.forEach(element=> {
        let title = element.product.name;
        let precio = Number(element.product.price).toLocaleString('es-CL');
    ...
    let finalTotal = total.toLocaleString('es-CL')
    let divFinal =`
    ...
    try{
        ...
        if(respuesta.status===200){
            ...
            datos.products.forEach((producto, i) => {
                ...
                const precio = Number(producto.price).toLocaleString('es-CL');
                cartas +=  `
...
const loadCinnamons = async() => {
    //VERIFICACION
    try{
        const respuesta = await fetch('http://localhost:8081/products/category/2');
        //FILTRANDO TIPO DE STATUS
        if(respuesta.status===200){
            const datos = await respuesta.json();
            //FOREACH PARA RECORRER EL ARRAY DEL API
...
const loadTortas = async() => {
    //VERIFICACION
    try{
        const respuesta = await fetch('http://localhost:8081/products/category/1');
        //FILTRANDO TIPO DE STATUS
        if(respuesta.status===200){
            const datos = await respuesta.json();
            //FOREACH PARA RECORRER EL ARRAY DEL API
...
const loadCupcakes = async() => {
    //VERIFICACION
    try{
        const respuesta = await fetch('http://localhost:8081/products/category/3');
        //FILTRANDO TIPO DE STATUS
        if(respuesta.status===200){
            const datos = await respuesta.json();
            //FOREACH PARA RECORRER EL ARRAY DEL API
...
const loadPie = async() => {
    //VERIFICACION
    try{
        const respuesta = await fetch('http://localhost:8081/products/category/4');
        //FILTRANDO TIPO DE STATUS
        if(respuesta.status===200){
            const datos = await respuesta.json();
            //FOREACH PARA RECORRER EL ARRAY DEL API
